...
function getAllMovieTitles(movies) {
    if (movies.length === 0){
      throw "Error: There is not any movie in the array."
    }
...
function checkIfAnyMovieHasRating(movies, rating = 'G') {
  if (movies.length === 0){
    throw "Error: There is not any movie in the movies' array."
  }
...
function filterByGenre(movies, genre) {
  ...
  return movieByGenre = movies.filter((movie) => movie.genre.toLowerCase().includes(genre.toLowerCase()));
}
...
function getAllMoviesReleasedAtOrBeforeYear(movies, year) {
  ...
  return movies.filter((movie) => Number(movie.released.slice(movie.released.length - 4, movie.released.length)) <= year);
}
